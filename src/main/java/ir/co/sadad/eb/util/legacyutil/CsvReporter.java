...
public class CsvReporter extends ScheduledReporter {
    ...
    private void report(long timestamp, String name, String header, String line, Object... values) {
        try {
            ...
            File todayDirectory = new File(this.directory, simpleDateFormat.format(new Date()));
            if (!todayDirectory.exists()) {
                todayDirectory.mkdirs();
            }
            File file = this.csvFileProvider.getFile(todayDirectory, name);
            ...
            if (fileAlreadyExists || file.createNewFile()) {
                ...
                try {
                    ...
                    out.printf(this.locale, String.format(this.locale, "%s" + this.separator + "%s%n", new Timestamp(timestamp * 1000).toString(), line), values);
